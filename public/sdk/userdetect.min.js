/**
 * UserDetect SDK v2.0.0
 * Browser-based user location detection with GPS geolocation.
...
var UserDetect = (function() {
    ...
    var DEFAULTS = {
        ...
        enableGeolocation: true,    // Request GPS location
        geolocationTimeout: 5000,   // GPS timeout in ms
        geolocationMaxAge: 300000   // Cache GPS for 5 minutes
    };
    ...
    function getGeolocation(options) {
        if (!options.enableGeolocation || !navigator.geolocation) {
            if (options.debug) console.log('[UserDetect] Geolocation not available or disabled');
            return Promise.resolve(null);
        }

        return new Promise(function(resolve) {
            var timeoutMs = options.geolocationTimeout || 5000;
            ...
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    ...
                    if (options.debug) console.log('[UserDetect] GPS coords:', coords);
                    resolve(coords);
                },
                function(error) {
                    if (options.debug) console.log('[UserDetect] Geolocation error:', error.message);
                    resolve(null); // Don't reject — fallback to IP
                },
                {
                    enableHighAccuracy: true,
                    timeout: timeoutMs,
                    maximumAge: options.geolocationMaxAge || 300000
                }
            );
        });
    }
    ...
    function collectSignals(fingerprint, geolocation) {
        var signals = {
    ...
    function init(apiKey, callback, options) {
        ...
        if (_options.debug) console.log('[UserDetect] Initialized v2.0.0', _options);
    ...
    function detect() {
        ...
        // Run fingerprint and GPS in parallel for speed
        return Promise.all([
            generateFingerprint(),
            getGeolocation(_options)
        ]).then(function(results) {
            _fingerprint = results[0];
            var geolocation = results[1];
            if (_options.debug) {
                console.log('[UserDetect] Fingerprint:', _fingerprint);
                console.log('[UserDetect] Geolocation:', geolocation ? 'acquired' : 'unavailable');
            }
            var signals = collectSignals(_fingerprint, geolocation);
